...
public class HexFormatter {
    ...
    private static void prependBytes(String hex) {
        ...
        if (hex.length() <= 0) {
            System.out.println("Error: Hex length is 0 after removing unnecessary characters");
        } else if (hex.length() <= 2) {
            System.out.println("Error: Hex length is less than 2 after removing unnecessary characters");
        } else if (hex.length() % 2 != 0) {
            System.out.println(String.format("Hex length %s is not even divisible by 2", hex.length()));
        } else {
            ArrayList<String> byteList = new ArrayList<>();

            for (int i = 2; i < hex.length(); i = i + 2) {
                byteList.add(hex.substring(i - 2, i));
            }

            formatForUnitTest(byteList);
            formatForSignalingTest(byteList);
        }
    }

    private static void formatForUnitTest(ArrayList<String> byteList) {

        StringBuilder sb = new StringBuilder();
        int valuesOnCurrentLine = 0;

        for (int i = 0; i < byteList.size(); i++) {

            String b = byteList.get(i);
            sb.append("(byte) 0x");
            sb.append(b);
            ...
            if (i != (byteList.size() - 1)) {
                ...
                if (++valuesOnCurrentLine == VALUES_PER_LINE) {
                    sb.append("\n");
                    valuesOnCurrentLine = 0;
                }
            }
        }
    ...
    private static void formatForSignalingTest(ArrayList<String> byteList) {
        ...
        for (int i = 0; i < byteList.size(); i++) {
            ...
            if (i != (byteList.size() - 1)
                    && ++valuesOnCurrentLine == (VALUES_PER_LINE * 2)) {
                sb.append("\n");
                valuesOnCurrentLine = 0;
            }
        }
        System.out.println("For Signaling Test...\n" + sb.toString());
        System.out.println();
    }
